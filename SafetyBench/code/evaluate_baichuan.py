...
def gen(path, outpath):
    ...
    model_path = "C:/Users/yyyzwyy/Desktop/SafetyBench/models/Baichuan-13B-Chat"  # 你的本地模型路径
    if not os.path.exists(model_path):
        raise ValueError(f"路径不存在: {model_path}")
    tokenizer = AutoTokenizer.from_pretrained("C:/Users/yyyzwyy/Desktop/SafetyBench/models/Baichuan-13B-Chat", use_fast=False, trust_remote_code=True,
                                              cache_dir="/mylocal")
    model = AutoModelForCausalLM.from_pretrained("C:/Users/yyyzwyy/Desktop/SafetyBench/models/Baichuan-13B-Chat", device_map="cpu",
                                                 torch_dtype=torch.bfloat16, trust_remote_code=True, offload_folder="offload",
                                                 cache_dir="/mylocal")
    model.generation_config = GenerationConfig.from_pretrained("C:/Users/yyyzwyy/Desktop/SafetyBench/models/Baichuan-13B-Chat", cache_dir="/mylocal")
    ...
    with open(outpath, 'a', encoding='utf-8') as outf:
        for start in trange(0, len(data), batch_size):
            print(f"Processing batch {start} to {start + batch_size} of {len(data)}")  # Debug 输出
            ...
            outputs = model.generate(**inputs, do_sample=True, max_new_tokens=32, min_new_tokens=1)
            responses = []
